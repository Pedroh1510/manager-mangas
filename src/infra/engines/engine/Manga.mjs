...
export default class Manga {
	...
	isChapterFileExisting(chapter) {
		...
		return (
			this.existingChapters[chapter.file.full] ||
	...
	getChapters(callback) {
		...
		Engine.Storage.getExistingChapterTitles(this)
			...
			.then((existingChapterTitles) => {
				...
				let onlineChapters = (chapter) =>
					chapter.status === statusDefinitions.completed ||
					chapter.status === statusDefinitions.available;
				return this.chapterCache && this.chapterCache.some(onlineChapters)
					? this._getUpdatedChaptersFromCache()
					: this._getUpdatedChaptersFromWebsite();
			})
			.then(() => {
				for (let existingChapterTitle in this.existingChapters) {
					if (!this.isChapterFileCached(existingChapterTitle)) {
						this.chapterCache.push(
							new Chapter(
								this,
								existingChapterTitle,
								existingChapterTitle,
								undefined,
								statusDefinitions.offline
							)
						);
					}
				}
