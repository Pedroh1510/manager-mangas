...
export default class MangaDex extends Connector {
	constructor() {
		...
		this.config = {
			throttleRequests: {
				...
				description:
					'Enter the timespan in [ms] to delay consecuitive requests to the api.',
				input: 'numeric',
				min: 100,
				max: 10000,
				value: 2000
		...
		this.serverNetwork = ['https://uploads.mangadex.org/data/'];
	}
	...
	async _initializeConnector() {
		...
		console.log(
			`Added Network Seeds '[ ${this.serverNetwork.join(', ')} ]' to ${
				this.label
			}`
		);
	...
	async _getMangaFromURI(uri) {
		...
		const id = (uri.pathname.match(regexGUID) ||
			uri.hash.match(regexGUID))[0].toLowerCase();
		const request = new Request(
			new URL('/manga/' + id, this.api),
			this.requestOptions
		);
		...
		return new Manga(
			this,
			id,
			data.attributes.title.en || Object.values(data.attributes.title).shift()
		);
	}

	async _getMangas() {
		return (
			await this.fetchJSON('https://websites.hakuneko.download/mangadex.json')
		).map((manga) => {
			return {
				id: manga.id,
				title: manga.title
		...
		return !data
			...
			: data
					.map((result) => {
						...
						const groups = result.relationships.filter(
							(r) => r.type === 'scanlation_group'
						);
						if (groups.length > 0) {
							title +=
								' [' +
								groups.map((group) => groupMap[group.id]).join(', ') +
								']';
						}
						...
						if (
							groups.length === 0 ||
							groups.some(
								(group) => !this.licensedChapterGroups.includes(group.id)
							)
						) {
							return {
								id: result.id,
	...
	async _handleConnectorURI(payload) {
		const servers = [...this.serverNetwork, payload.networkNode];
		for (let node of servers) {
			try {
				...
				if (response.ok && response.status === 200) {
					...
					if (
						response.headers.get('content-length') == data.size ||
		...
		throw new Error(
			'Failed to download image file from MangaDex@Home network!\n' +
				payload.networkNode
		);
	}
	...
	async _getScanlationGroups(chapters) {
		const groupList = {};
		let groupIDs = !chapters
			? {}
			: chapters.reduce((accumulator, chapter) => {
		...
		if (groupIDs.length > 0) {
			...
			uri.search = new URLSearchParams([
				['limit', 100],
				...groupIDs.map((id) => ['ids[]', id])
			]).toString();
			const request = new Request(uri, this.requestOptions);
			const { data } = await this.fetchJSON(request, 3);
			data.forEach(
				(result) => (groupList[result.id] = result.attributes.name || 'unknown')
			);
		}
		return groupList;
	}
